...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  squared_error = tf.squared_difference(pred,labels)
  squared_error = tf.reduce_sum(squared_error,1)
  mse = tf.reduce_mean(squared_error)

  # Create a tensor named mse for logging purposes.
  tf.identity(mse, name='mse')
  ...
  if not tf.contrib.distribute.has_distribution_strategy():
    accuracy = tf.metrics.mean(squared_error)
  else:
...
def resnet_main(
  ...
  def input_fn_train(num_epochs):
    return input_function(
        mode="train", data_dir=flags_obj.data_dir,
  ...
  def input_fn_eval():
    return input_function(
        mode="validate", data_dir=flags_obj.data_dir,
        batch_size=per_device_batch_size(
            flags_obj.batch_size, flags_core.get_num_gpus(flags_obj)),
        num_epochs=1)
  def input_fn_pred():
    return input_function(
        mode="predict", data_dir=flags_obj.data_dir,
        batch_size=per_device_batch_size(
            flags_obj.batch_size, flags_core.get_num_gpus(flags_obj)),
        num_epochs=1)
  ...
  for cycle_index, num_train_epochs in enumerate(schedule):
    ...
    if model_helpers.past_stop_threshold(
        flags_obj.stop_threshold, eval_results['mse']):
      break
...
def define_resnet_flags(resnet_size_choices=None):
  ...
  flags.DEFINE_boolean(
      name='predict_only', default=False,
      help=flags_core.help_wrap('Skip training and only perform prediction on '
                                'the latest checkpoint.'))
